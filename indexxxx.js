...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    let vlessURL = ``;
    let trojanURL = ``;
    for (let i = 0; i < ProxyDomain.length; i++) {
      vlessURL += `vless://${UUID}@${ProxyDomain[i]}:443?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${namePart}` + '\n';
      trojanURL += `trojan://${UUID}@${ProxyDomain[i]}:443?security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${namePart}` + '\n';
    }
